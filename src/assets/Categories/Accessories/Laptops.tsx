...
export default function Laptops() {
  ...
  useEffect(() => {
    const fetchAndDuplicateLaptops= async () => {
      try {
        setLoading(true);
        ...
        const baseLaptopsItems: DummyJsonProduct[] = data.products; 

        if (!baseLaptopsItems || baseLaptopsItems.length === 0) {
          setError("No Laptops items found.");
          setProducts([]);
          return;
        }
        ...
        while (currentCount < TARGET_ITEM_COUNT) {
          for (const item of baseLaptopsItems) {
