...
export default function SmartPhones() {
  ...
  useEffect(() => {
    const fetchAndDuplicateSmartPhones= async () => {
      try {
        setLoading(true);
        ...
        const baseSmartPhonesItems: DummyJsonProduct[] = data.products; 

        if (!baseSmartPhonesItems || baseSmartPhonesItems.length === 0) {
          setError("No SmartPhones items found.");
          setProducts([]);
          return;
        }
        ...
        while (currentCount < TARGET_ITEM_COUNT) {
          for (const item of baseSmartPhonesItems) {
        ...
        setError("Failed to load SmartPhones. Please try again.");
      } finally {
        setLoading(false);
      }
    };

    fetchAndDuplicateSmartPhones();
  }, []);
