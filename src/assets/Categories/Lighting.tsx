...
export default function Lighting() {
  ...
  useEffect(() => {
    const fetchAndDuplicateLighting = async () => {
      try {
        setLoading(true);
        ...
        const baseLightingItems: DummyJsonProduct[] = data.products; 

        if (!baseLightingItems || baseLightingItems.length === 0) {
          setError("No Lighting items found.");
          setProducts([]);
          return;
        }
        ...
        while (currentCount < TARGET_ITEM_COUNT) {
          for (const item of baseLightingItems) {
        ...
        console.error("Error fetching Lighting:", err);
        setError("Failed to load Lighting. Please try again.");
      } finally {
        setLoading(false);
      }
    };

    fetchAndDuplicateLighting();
  }, []);
