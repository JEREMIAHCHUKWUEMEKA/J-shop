...
export default function WomensDresses() {
  ...
  useEffect(() => {
    const fetchAndDuplicateWomensDresses= async () => {
      try {
        setLoading(true);
        ...
        const baseWomensDressesItems: DummyJsonProduct[] = data.products; 

        if (!baseWomensDressesItems || baseWomensDressesItems.length === 0) {
          setError("No WomensDresses items found.");
          setProducts([]);
          return;
        }
        ...
        while (currentCount < TARGET_ITEM_COUNT) {
          for (const item of baseWomensDressesItems) {
        ...
        console.error("Error fetching WomensDresses:", err);
        setError("Failed to load WomensDresses. Please try again.");
      } finally {
        setLoading(false);
      }
    };

    fetchAndDuplicateWomensDresses();
  }, []);
