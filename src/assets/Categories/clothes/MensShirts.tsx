...
export default function MensShirts() {
  ...
  useEffect(() => {
    const fetchAndDuplicateMensShirts= async () => {
      try {
        setLoading(true);
        ...
        const baseMensShirtsItems: DummyJsonProduct[] = data.products; 

        if (!baseMensShirtsItems || baseMensShirtsItems.length === 0) {
          setError("No MensShirts items found.");
          setProducts([]);
          return;
        }
        ...
        while (currentCount < TARGET_ITEM_COUNT) {
          for (const item of baseMensShirtsItems) {
        ...
        console.error("Error fetching MensShirts:", err);
        setError("Failed to load MensShirts. Please try again.");
      } finally {
        setLoading(false);
      }
    };

    fetchAndDuplicateMensShirts();
  }, []);
