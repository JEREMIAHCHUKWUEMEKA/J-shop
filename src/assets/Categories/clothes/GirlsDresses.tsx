...
export default function GirlsDresses() {
  ...
  useEffect(() => {
    const fetchAndDuplicateGirlsDresses= async () => {
      try {
        setLoading(true);
        ...
        const baseGirlsDressesItems: DummyJsonProduct[] = data.products; 

        if (!baseGirlsDressesItems || baseGirlsDressesItems.length === 0) {
          setError("No GirlsDresses items found.");
          setProducts([]);
          return;
        }
        ...
        while (currentCount < TARGET_ITEM_COUNT) {
          for (const item of baseGirlsDressesItems) {
        ...
        console.error("Error fetching GirlsDresses:", err);
        setError("Failed to load GirlsDresses. Please try again.");
      } finally {
        setLoading(false);
      }
    };

    fetchAndDuplicateGirlsDresses();
  }, []);
