...
export default function MensShoes() {
  ...
  useEffect(() => {
    const fetchAndDuplicateMensShoes= async () => {
      try {
        setLoading(true);
        ...
        const baseMensShoesItems: DummyJsonProduct[] = data.products; 

        if (!baseMensShoesItems || baseMensShoesItems.length === 0) {
          setError("No MensShoes items found.");
          setProducts([]);
          return;
        }
        ...
        while (currentCount < TARGET_ITEM_COUNT) {
          for (const item of baseMensShoesItems) {
        ...
        console.error("Error fetching MensShoes:", err);
        setError("Failed to load MensShoes. Please try again.");
      } finally {
        setLoading(false);
      }
    };

    fetchAndDuplicateMensShoes();
  }, []);
