...
export default function MotorCycle() {
  ...
  useEffect(() => {
    const fetchAndDuplicateMotorCycle = async () => {
      try {
        setLoading(true);
        ...
        const baseMotorCycleItems: DummyJsonProduct[] = data.products; 

        if (!baseMotorCycleItems || baseMotorCycleItems.length === 0) {
          setError("No MotorCycle items found.");
          setProducts([]);
          return;
        }
        ...
        while (currentCount < TARGET_ITEM_COUNT) {
          for (const item of baseMotorCycleItems) {
        ...
        setError("Failed to load MotorCycle. Please try again.");
      } finally {
        setLoading(false);
      }
    };

    fetchAndDuplicateMotorCycle();
  }, []);
