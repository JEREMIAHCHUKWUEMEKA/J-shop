...
export default function Automotive() {
  ...
  useEffect(() => {
    const fetchAndDuplicateAutomotive = async () => {
      try {
        setLoading(true);
        ...
        const baseAutomotiveItems: DummyJsonProduct[] = data.products; 

        if (!baseAutomotiveItems || baseAutomotiveItems.length === 0) {
          setError("No Automotive items found.");
          setProducts([]);
          return;
        }
        ...
        while (currentCount < TARGET_ITEM_COUNT) {
          for (const item of baseAutomotiveItems) {
        ...
        console.error("Error fetching Automotive:", err);
        setError("Failed to load Automotive. Please try again.");
      } finally {
        setLoading(false);
      }
    };

    fetchAndDuplicateAutomotive();
  }, []);
