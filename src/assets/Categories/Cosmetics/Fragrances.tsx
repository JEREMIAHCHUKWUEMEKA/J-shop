...
export default function Fragrances() {
  ...
  useEffect(() => {
    const fetchAndDuplicateFragrances = async () => {
      try {
        setLoading(true);
        ...
        const baseFragrancesItems: DummyJsonProduct[] = data.products; 

        if (!baseFragrancesItems || baseFragrancesItems.length === 0) {
          setError("No Fragrances items found.");
          setProducts([]);
          return;
        }
        ...
        while (currentCount < TARGET_ITEM_COUNT) {
          for (const item of baseFragrancesItems) {
